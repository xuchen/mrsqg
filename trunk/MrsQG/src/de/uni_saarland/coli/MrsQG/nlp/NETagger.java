package de.uni_saarland.coli.MrsQG.nlp;

import de.uni_saarland.coli.MrsQG.util.FileUtils;
import de.uni_saarland.coli.MrsQG.util.StringUtils;
...
public class NETagger {
	...
	public static boolean loadNameFinders(String dir) {
		...
		try {
		    for (int i = 0; i < files.length; i++) {
				...
				System.out.println("    ...for " + finderNames[i]);
		    }
		} catch (IOException e) {
			return false;
		}
	...
	static {
		loadListTaggers("./res/nlp/netagger/lists/");
	}
	...
	public static void loadListTaggers(String listDirectory) {
		...
		for (File list : listFiles) {
			...
			System.out.println("    ...for NE" + listName);
		}
	...
	static {
		loadRegExTaggers("./res/nlp/netagger/patterns.lst");
	}
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				try {
					...
					System.out.println("    ...for " + neName);
				} catch (Exception e) {
					System.out.println("    ...could not add " + neName);
				}
			}
